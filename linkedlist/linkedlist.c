...
int *append(Value *list, int idx) {
    if (idx < 0 || idx > length(list)) {
        return 0;
    }
    
    if (idx == 0) {
        Value *newNode = cons(newNode, list); //Assign cdr of the new node to the old head
        list = newNode; //Assign head = newNode
    }
}
...
Value *reverse(Value *list) {
    if(list->type == CONS_TYPE) {
        ...
        while (oldList->type == CONS_TYPE) {
            Value *carNode = malloc(sizeof(Value));
            switch (car(oldList)->type) {
                ...
                case (STR_TYPE):
                    ...
                    carNode->s = malloc(sizeof(char) * (strlen(car(oldList)->s) + 1));
    ...
    Value *singleList = malloc(sizeof(Value));
    switch (list->type) {
        case (INT_TYPE):
